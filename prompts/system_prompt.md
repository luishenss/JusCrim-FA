...
**[OBRIGATÓRIO]** Liste TODOS os processos identificados mas NÃO considerados como condenação válida:
...
Confirme antes de entregar a análise:

- [ ] Li TODAS as páginas antes de iniciar qualquer análise
- [ ] Analisei processos originais + execuções em conjunto
- [ ] Encontrei literalmente variações de "Trânsito em julgado para a Defesa"
- [ ] Para cada processo INCLUÍDO: confirmei sentença condenatória + trânsito em julgado para defesa
- [ ] Para cada processo EXCLUÍDO: expliquei especificamente por que não atende aos critérios
- [ ] Apliquei o CHECKPOINT TEMPORAL para cada condenação
- [ ] Calculei corretamente todas as datas usando art. 10 do CP
- [ ] Verifiquei se extinções ocorreram APÓS condenações válidas
- [ ] Considerei processos com extinção/indulto APÓS condenação válida
- [ ] Citei ambos os números de processo quando aplicável
- [ ] Listei TODOS os processos da certidão (incluídos + excluídos)
- [ ] Ordenei condenações por ordem de aparição na certidão
- [ ] Combinei informações distribuídas entre seções relacionadas
